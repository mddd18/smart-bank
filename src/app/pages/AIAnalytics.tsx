...
import { BrainCircuit, AlertTriangle, Activity, ShieldCheck, ArrowRight, Loader2 } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from 'recharts';
import { Asset, Employee } from '../types';
import { getAssets, getEmployees } from '../utils/storage';
...
export function AIAnalytics() {
  ...
  const loadAnalytics = async () => {
    setLoading(true);
    try {
      // Ma'lumotlarni Supabase'dan kutib olamiz
      ...
      console.error("Tahlil ma'lumotlarini olishda xatolik:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh]">
        ...
        <p className="text-gray-500">AI ma'lumotlarni tahlil qilmoqda, kuting...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl lg:text-3xl mb-1 text-indigo-950 font-bold">AI Tahlil va Bashoratlar</h1>
          <p className="text-gray-600 text-sm">Sun'iy intellekt orqali jihozlarning buzilish xavfini oldindan aniqlash</p>
        </div>
      </div>
            ...
            <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-lg flex gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-indigo-700">
                  Bo'limlardagi aktivlarning amortizatsiya muddati tahlil qilindi. Jihozlarni muntazam yangilab turish operatsion xarajatlarni kamaytiradi.
                </p>
              </div>
            </div>
          </div>
